...
class MoDEAgent(pl.LightningModule):
    ...
    def compute_input_embeddings(self, dataset_batch):
        ...
        if self.use_image_text_not_embedding:
            ...
            latent_goal = dataset_batch["vis"].to(rgb_static.dtype) # vis emb conditioned on lang_text emb
    ...
    def forward(self, obs, goal):
        ...
        if self.use_image_text_not_embedding:
            ...
            latent_goal = goal["vis"].unsqueeze(0).to(torch.float32).to(rgb_static.device) # vis emb conditioned on lang_text emb
